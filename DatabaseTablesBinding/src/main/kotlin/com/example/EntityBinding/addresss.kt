...
object addresss:Table<address>("address") {
    val addressid=int("id").primaryKey().bindTo { it.addressid }
    ...
    val created_at=varchar("created_at_ad").references(customers){it.created_at}
    val updated_at=varchar("updated_at_ad").references(customers){it.updated_at}
}